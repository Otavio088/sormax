...
def calculate():
    ...
    try:
        ...
        # Verificar se existem produtos selecionados
        if not products:
        ...
        # Agora podemos montar o problema de maximização
        ...
        # Restrições (não ultrapassar insumos disponíveis)
        ...
        # Obter valores ótimos
        quantities = [round(pulp.value(var) or 0, 2) for var in x]
        max_profit = round(pulp.value(prob.objective) or 0)
        
        # Calcular uso de insumos
        used_restrictions = [
            round(sum(quantities[j] * necessary_restrictions[i][j] for j in range(len(products))), 2)
            for i in range(len(restrictions))
        ]
        ...
        chart_data = [
            {"name": product.name, "y": quantities[i]}
            for i, product in enumerate(products)
        ]
